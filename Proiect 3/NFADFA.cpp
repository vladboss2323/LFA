...
void NFADFA :: citire_gramatica()
{
    int i,nr_noduri = 0;
    map<char,int>noduri;
    vector<pair<int,char>>tranz_nod_final;
    char tranzitie[10];
    q0 = 0;
    while(in.getline(tranzitie,5))
    {
        //cout<<tranzitie<<'\n';
        char litera = tranzitie[2];
        if(noduri.find(tranzitie[0]) == noduri.end())
        {
            noduri[tranzitie[0]] = nr_noduri;
            nr_noduri++;
        }
        int nod = noduri[tranzitie[0]];
        if(tranzitie[3])
        {
            cout<<tranzitie[0] << " " <<tranzitie[3] <<'\n';
            if(!noduri[tranzitie[3]])
            {
                noduri[tranzitie[3]] = nr_noduri;
                nr_noduri++;
            }
            int nod_dest = noduri[tranzitie[3]];
            cout<<nod << " " << nod_dest<<'\n';
            a[nod].push_back(make_pair(nod_dest,litera));
        }
        else
        {
            if(litera == '~')
            ...
            {
                tranz_nod_final.push_back(make_pair(nod,litera));
            }
        }
    }
    qf[nr_noduri] = true;
    for(auto x : tranz_nod_final)
    {
        a[x.first].push_back(make_pair(nr_noduri ,x.second));
    }
    n = nr_noduri + 1;
}
